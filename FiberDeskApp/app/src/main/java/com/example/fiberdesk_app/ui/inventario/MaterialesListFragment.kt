...
class MaterialesListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Check if filtering by low stock
        val filtroBajoStock = arguments?.getBoolean("filtro_bajo_stock", false) ?: false

        // Configurar toolbar
        activity?.title = if (filtroBajoStock) "Materiales Bajo Stock" else "Gestionar Materiales"
        ...
        val fabAdd = view.findViewById<FloatingActionButton>(R.id.fab_add_material)
        ...
        // Cargar materiales
        loadMaterials(filtroBajoStock)
    }

    private fun loadMaterials(filtroBajoStock: Boolean = false) {
        ...
        lifecycleScope.launch {
            try {
                ...
                if (filtroBajoStock) {
                    materials.addAll(response.filter { it.cantidad < 10 })
                } else {
                ...
                if (materials.isEmpty()) {
                    ...
                    emptyMessage.text = if (filtroBajoStock) {
                        "No hay materiales bajo stock"
                    } else {
    ...
    private fun addMaterial(nombre: String, cantidad: Int, descripcion: String) {
        ...
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(requireContext(), "Material agregado", Toast.LENGTH_SHORT).show()
                loadMaterials()
            } catch (e: Exception) {
                Toast.makeText(
                    requireContext(),
